...
{
    public class RightAngleMethodExperiments : ExperimetBase
    {
        public RightAngleMethodExperiments(ITestOutputHelper output) : base(output)
        {
            Log("Метод правый уголок");
        }
        ...
        {
            Log("Одна переменная по пространс1тву, с постоянным запаздыванием");
            Log("x: (0, 2), t: (-1, 1)");
            Log($"N = {n}; M = {m}");
            ...
            var method = new RightAngleMethod(OneSpatialVariable_ConstantDelay_ExperimentFunction, new LinearInterpolationService());
        ...
        //    var method = new RightAngleMethod(OneSpatialVariable_VariableDelay_ExperimentFunction, new LinearInterpolationService());
        //    var solver = new SequentialRunner(method);
